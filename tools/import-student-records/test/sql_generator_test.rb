...
class SQLGeneratorTest < Test::Unit::TestCase
  ...
  def test_should_be_able_to_cope_with_missing_values
    ...
    sqls = @generator.generate_sql([[student,Talent.new('')]])
    sql = sqls.first
    assert_equal("INSERT INTO student (name) VALUES ('minno');", sql)
  end
  ...
  def test_should_generate_multiple_insert_statements
    ...
    sql_statements = @generator.generate_sql([[minno,Talent.new('')],[pat,Talent.new('')]])

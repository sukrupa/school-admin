...
//		System.out.println("tempVarName="+tempVarName);		
	...
	public void test2Params(){
		assertEquals("_assertEqual(\\\"Rs. 18\\\", \"+s_v($table.get(\"Soap\", \"Price\"))+\");", 
				testScript.separateVariables("_assertEqual(\"Rs. 18\", $table.get(\"Soap\", \"Price\"));"));
		assertEquals("_assertEqual(\\\"Rs. 18\\\", \"+s_v($table.get(\"Soap\",\t\"Price\"))+\");", 
				testScript.separateVariables("_assertEqual(\"Rs. 18\", $table.get(\"Soap\",\t\"Price\"));"));
	}
	...
	public void testSeparateVariablesNoDollar(){
		assertEquals("_setValue(_textbox(1), \\\"url\\\")", testScript.separateVariables("_setValue(_textbox(1), \"url\")"));
	}
	...
	public void testSeparateVariablesWithDollar(){
		assertEquals("_setValue(_textbox(1), \"+s_v($url)+\")", testScript.separateVariables("_setValue(_textbox(1), $url)"));
	}
	
	public void testSeparateVariablesWith2Dollar(){
		assertEquals("\"\"+s_v($a)+\"==\"+s_v($b)+\"\"", "\"" + testScript.separateVariables("$a==$b") + "\"");
		assertEquals("\"_setValue(\"+s_v($a)+\", \"+s_v($b)+\")\"", "\"" + testScript.separateVariables("_setValue($a, $b)") + "\"");
	}

	public void testSeparateVariablesWithRegExp(){
		assertEquals("_assertEqual(\"+s_v($fullFilePath.replace(/\\/g, '/'))+\", 'a');", testScript.separateVariables("_assertEqual($fullFilePath.replace(/\\/g, '/'), 'a');"));
		assertEquals("_assertEqual(\"+s_v($fullFilePath.replace(/\\/g, '/'))+\", \"+s_v($resolvedPath.replace(/\\/g, '/'))+\");", testScript.separateVariables("_assertEqual($fullFilePath.replace(/\\/g, '/'), $resolvedPath.replace(/\\/g, '/'));"));
	}

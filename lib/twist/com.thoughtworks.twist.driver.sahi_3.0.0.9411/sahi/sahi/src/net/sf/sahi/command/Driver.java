...
public class Driver {
	...
	private void launchBrowser(final HttpRequest request) {
        ...
        final BrowserLauncher launcher = new BrowserLauncher(browser, browserProcessName, browserOption);
		String url = "http://sahi.example.com/_s_/dyn/Driver_start?sahisid="
			+ session.id()
			+ "&startUrl="
			+ Utils.encode("http://sahi.example.com/_s_/dyn/Driver_initialized?startUrl="+Utils.encode(startUrl));    	

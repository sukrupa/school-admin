...
if (_sahi.controllerMode == "testmaker"){
	...
	Sahi.prototype.getStepObj = function(action, accessor, value, dialect){
	...
	Sahi.prototype.getScript = function (infoAr, el) {
		...
		for (var i=0; i<locators.length; i++){
			toSendAr[toSendAr.length] = this.getStepObj(action, this.quotedEscapeValue(locators[i][0]), value, "selenium");
		}
...
//		this._alert(this.toJSON(toSendAr));
	...
	Sahi.prototype.sendIdentifierInfo = function(accessors, escapedAccessor, escapedValue, popupName){
		...
		var json = this.toJSON({accessors: {sahi: accessors, selenium: selAccessors}, assertions: assertions, value: escapedValue});
...
//	Sahi.prototype.openController = function(){};	
	_sahi.language = {
			...
			ASSERT_VISIBLE: "assert__xxSAHIDIVIDERxx__isVisible(<accessor>)__xxSAHIDIVIDERxx__",
			...
			ASSERT_NOT_CHECKED: "assertNotTrue__xxSAHIDIVIDERxx__<accessor>.checked__xxSAHIDIVIDERxx__",
			POPUP: "popup(<window_name>).",
			DOMAIN: "domain(<domain>)."		
	};		
}
